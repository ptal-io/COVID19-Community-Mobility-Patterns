	...
	for(cat in cats) {
		...
		if (file.exists(googlefile) & file.exists(oxfile)) { # & file.exists(afile) & file.exists(bfile)) {
			...
			if (!all(is.na(ovals)) & !all(is.na(gvals))) {
				...
				a <- cbind(mm[,1],seq(1:length(mm[,1])))
				b <- cbind(mm[,2],seq(1:length(mm[,2])))
				f <- hausdorff_dist(a,b)
				
				m[cnt,colcnt] <- round(f, digits=5)
			}
		}
		colcnt <- colcnt +1
	}
...
write.table(newdf, 'gs_hausdorff.csv', col.names=TRUE, row.names=FALSE)
...
write.table(df2, 'gs_hausdorff_avgs.csv', col.names=TRUE, row.names=FALSE)

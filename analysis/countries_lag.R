	...
	for(cat in cats) {
		...
		if (file.exists(googlefile) & file.exists(oxfile)) { # & file.exists(afile) & file.exists(bfile)) {
			...
			if (length(unique(ovals)) > 1 && length(unique(gvals)) > 1) {
				...
				ccf <- ccf(mm[,1], mm[,2], lag=30, plot=FALSE)
				low <- ccf$acf[1:30]
				high <- ccf$acf[32:61]

				t3 <- length(low[low>0.3]) - length(high[high>0.3])
				m[cnt,colcnt] <- t3
			}
		}
		colcnt <- colcnt +1
...
write.table(newdf, 'gs_lag.csv', col.names=TRUE, row.names=FALSE)
...
write.table(df2, 'gs_lag_avgs.csv', col.names=TRUE, row.names=FALSE)

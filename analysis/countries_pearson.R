	...
	for(cat in cats) {
		...
		if (file.exists(googlefile) & file.exists(oxfile)) { # & file.exists(afile) & file.exists(bfile)) {
			...
			if (length(unique(ovals)) > 1 & length(unique(gvals)) > 1) {
				...
				cor = cor(na.omit(mm), method='pearson')
				
				m[cnt,colcnt] <- round(cor[1,2], digits=5)
			}
		}
		colcnt <- colcnt +1
	}
...
write.table(newdf, '../internalData/gs_pearson.csv', col.names=TRUE, row.names=FALSE, sep=",")
...
write.table(df2, '../internalData/gs_pearson_avgs.csv', col.names=TRUE, row.names=FALSE)

...
for(cntry in ucountries) {
	...
	oxfile = paste("oxvars/",cntry,".csv",sep="")
	m[cnt,1] <- cntry
	colcnt <-2
	for(cat in cats) {
		...
		if (file.exists(googlefile) & file.exists(oxfile)) { # & file.exists(afile) & file.exists(bfile)) {
			...
			o <- read.csv(oxfile, header=T)
			o[is.na(o)] <- 0
			odates <- o$Date
			ovals <- o$S3_Cancelpublicevents + o$S3_IsGeneral * 33.3333333333
...
newdf$avg = rowMeans(newdf[,c("parks", "retailrecreation", "grocerypharmacy","transitstations","residential","workplace")], na.rm=TRUE)
write.table(newdf, 'gs_publicevents.csv', col.names=TRUE, row.names=FALSE)


newdf[newdf$country == 'MN',] <- c("MN",NA,NA,NA,NA,NA,NA,NA)
newdf[newdf$country == 'KR',] <- c("MN",NA,NA,NA,NA,NA,NA,NA)
rm <- vector()
cnt <- 1;
for(i in 1:length(newdf[,1])) {
	if(all(is.na(newdf[i,2:7]))) {
		rm[cnt] <- i
		cnt <- cnt + 1
	}
}
...
write.table(df2, 'gs_publicevents_avgs.csv', col.names=TRUE, row.names=FALSE)

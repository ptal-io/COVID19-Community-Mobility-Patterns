	...
	for(cat in cats) {
		...
		if (file.exists(googlefile) & file.exists(oxfile)) { # & file.exists(afile) & file.exists(bfile)) {
			...
			if (length(unique(ovals)) > 1 && length(unique(gvals)) > 1) {
				...
				ccf <- ccf(mm[,1], mm[,2], lag=10, plot=FALSE)
				t1 <- sum(ccf$acf[1:10])
				t2 <- sum(ccf$acf[12:21])
				t3 <- t1/t2
				index <- 11 - which(ccf$acf==max(ccf$acf))
				m[cnt,colcnt] <- index
			}
		}
		colcnt <- colcnt +1
...
write.table(newdf, 'gs_lag_days.csv', col.names=TRUE, row.names=FALSE)
...
write.table(df2, 'gs_cor_avgs.csv', col.names=TRUE, row.names=FALSE)
